...
class WMD_MSReader_Module_Follow extends WMD_MSReader_Modules {
    ...
    function list_control($blog_id = false, $lists_ids = array(), $new_list_name = '') {
        ...
        if(is_numeric($blog_id) && $blog_id) {
            ...
            if($blog_details){
                ...
                $this->message_type = 1;
                $this->message = __( 'Blog-Listen aktualisiert.', 'wmd_msreader' );
    ...
    function list_manage() {
        if(isset($_POST['msreader_follow_manage']) && wp_verify_nonce($_POST['msreader_follow_manage'], 'msreader_follow_manage') && isset($_POST['follow_list'])) {
            ...
            if(isset($_POST['change_name'])) {
                $this->message_type = 1;
                $this->message = __( 'Listenname geändert.', 'wmd_msreader' );
            ...
            elseif(isset($_POST['delete'])) {
                ...
                if(isset($_POST['delete_confirm']) && $_POST['delete_confirm']) {
                    ...
                    $this->message_type = 1;
                    $this->message = __( 'Liste gelöscht.', 'wmd_msreader' );
            ...
            if(!isset($_POST['new_list_name']) || !$_POST['new_list_name']) {
                ...
                if(!$name_exists_key) {
                    ...
                    if(isset($_POST['list_blogs_links']) && $_POST['list_blogs_links']) {
                        ...
                        $this->message = sprintf( _n('Liste mit %d Blog wurde erstellt.', 'Liste mit %d Blogs wurde erstellt.', count($count), 'wmd_msreader'), count($blogs_ids));
                        $redirect_url = $this->get_module_dashboard_url($new_list_id);
                    }
                    else {
                        $this->message_type = 1;
                        $this->message = __( 'Liste wurde erstellt.', 'wmd_msreader' );
        ...
        if(is_numeric($blog_id) && $blog_id) {
            ...
            ?>
                <div class="msreader-popup-inside">
                    <h4><?php _e('Wähle Listen für diese Webseite aus', 'wmd_msreader'); ?></h4>
            ...
            <hr/>
            <div class="msreader-popup-inside">
                <button data-nonce="<?php echo wp_create_nonce( 'manage-follow-save' ); ?>" class="button button-primary right msreader-manage-follow-save"><?php _e('Speichern', 'wmd_msreader'); ?></button>
            </div>
        <?php
        }
    ...
    function add_widget($widgets) {
        ...
        foreach ($user_follow_data_lists as $list_id => $list_name) {
            ...
            $lists[] = array('args' => $list_id,'title' => $list_name.'</a><a '.$active.'href="'.$link.'" title="'.__('Verwalte diese Liste', 'wmd_msreader').'"><span class="msreader-widget-links-element msreader-widget-links-icon dashicons-admin-generic"></span>');
        }
                ...
                'title' => 
                    '<div class="msreader-create-list msreader-popup-container">
                        <a class="add-new-h2 msreader-show button-small msreader-create-list-button" title="Create new list" href="#">
                            ...
                            '.__('Neue Liste erstellen', 'wmd_msreader').'
                        </a>
                        <div class="msreader-create-list-form msreader-popup">
                            ...
                            <div class="msreader-popup-content">
                                <form method="post" action="">
                                    ...
                                    <hr/>
                                    <div class="msreader-popup-inside">
                                        <input type="submit" class="button button-primary right" name="create_list" value="'.__('Erstellen', 'wmd_msreader').'"/>
                                    </div>
                                </form>
                            </div>
                        </div>
        ...
        $widgets['my-follow-lists'] = $this->create_list_widget($lists, array('title' => __('Meine Listen', 'wmd_msreader')));
    ...
    function widget_recent_posts_add_arg_modules($arg_modules) {
        ...
        asort($user_follow_data_lists, SORT_NATURAL);
        foreach ($user_follow_data_lists as $list_id => $list_name) {
            $arg_modules[] = array('class' => 'my_lists', 'value' => $this->details['slug'].'|'.$list_id, 'title' => __('Meine Listen', 'wmd_msreader').': '.$list_name);
        }
    ...
    function get_page_title() {
        ...
        if(isset($this->args['action']) && $this->args['action'] == 'manage') {
            if(isset($list_name)) {
                $title = __( 'Meine Listen', 'wmd_msreader' ).': <span>'.__( 'Verwalten', 'wmd_msreader' ).' - '.$list_name.'</span>';
            }
            else
                $title = __( 'Verfolgte Webseiten', 'wmd_msreader' ).': <span>'.__( 'Verwalten', 'wmd_msreader' ).'</span>';
        }
        elseif(isset($list_name)) {
            $title = __( 'Meine Listen', 'wmd_msreader' ).': <span>'.$list_name.'</span>';
        }
        else
            $title = __( 'Verfolgte Webseiten', 'wmd_msreader' );
        ...
        if(isset($this->args['action']) && ($this->args['action'] == 'manage' || $this->args['action'] == 'unfollow')) {
            ...
            if($followed_by_user) {
                ...
                if(isset($follow_list)) {
                    $title = ' '.__( 'Verwalte Webseiten in dieser Liste', 'wmd_msreader' );
                    $this->user_follow_data = $this->get_user_follow_data();
    ...
    function add_options_html($blank, $options) {
        $button_visibility_on_options = array('both' => __( 'Frontend- und Backend-Bereich', 'wmd_msreader' ), 'front' => __( 'Frontend-Bereich', 'wmd_msreader' ), 'back' => __( 'Backend-Bereich', 'wmd_msreader' ));
