<div class="theme-header msreader-post-header-navigation">
	<button class="left dashicons dashicons-no"><span class="screen-reader-text"><?php _e( 'Vorherigen Beitrag anzeigen', 'wmd_msreader' ); ?></span></button>
	<button class="right dashicons dashicons-no"><span class="screen-reader-text"><?php _e( 'Nächsten Beitrag anzeigen', 'wmd_msreader' ); ?></span></button>
	<div class="links">
		<a href="<?php echo get_permalink($post->ID); ?>"><?php _e( 'Original anzeigen', 'wmd_msreader' ); ?></a><?php
		if(current_user_can( 'edit_others_posts', $post->ID ) || ($post->post_author == $current_user_id))
			 edit_post_link(__( 'Bearbeiten', 'wmd_msreader' ), '', '');
		if($post->post_status != 'publish' && current_user_can( 'publish_posts', $post->ID ))
			echo '<button class="publish" data-nonce="'.wp_create_nonce( 'publish_post' ).'">'.__( 'Veröffentlichen', 'wmd_msreader' ).'</button>';
		echo apply_filters('msreader_dashboard_single_links', '', $post);
		?>
	</div>
	<button class="close dashicons dashicons-no"><span class="screen-reader-text"><?php _e( 'Overlay schließen', 'wmd_msreader' ); ?></span></button>
...
<div class="msreader-post">
	...
	<div class="msreader-comments">
		...
		<div class="comments-loader">
			<button class="button button-secondary load-previous-comments"><?php _e('Vorherige Kommentare laden', 'wmd_msreader' ); ?></button>
			<span class="spinner spinner-save"></span>
		</div>
		<?php } ?>
	...
	<div class="msreader-add-comment">
		<?php if(comments_open( $post->ID )) { ?>
			<form action="" method="post" class="add-comment-form">
				<div class="add-comment-info">
					<h4>
						<?php _e('Einen Kommentar hinzufügen', 'wmd_msreader' ); ?> 
				...
				<p class="form-submit">
					<span class="reply-info" style="display:none;"><small><?php _e('Reply to', 'wmd_msreader' ); ?>:</small> <strong class="reply-parent-name"></strong> <small>(<a class="reply-cancel" href="#"><?php _e('Abbrechen', 'wmd_msreader' ); ?></a>)</small></span>
		...
		<?php } else { ?>
			<div id="msreader-comments-closed">
				<?php _e( 'Kommentarfunktion ist geschlossen.', 'wmd_msreader' ); ?>
			</div>
		<?php } ?>
	</div>
</div>