	...
	<article data-comment_id="<?php comment_ID(); ?>" id="comment-<?php comment_ID(); ?>" class="comment-body<?php echo ($comment->comment_approved == '0') ? ' unapproved' : ' approved'; echo ($comment->comment_type == 'private') ? ' private' : ' public'; ?>">
		<footer class="comment-meta">
			...
			<div class="comment-metadata">
				<time datetime="<?php comment_time( 'c' ); ?>">
					<?php printf( _x( '%1$s um %2$s', '1: date, 2: time', 'wmd_msreader' ), get_comment_date(), get_comment_time() ); ?>
				</time>
			</div>
		</footer>
		...
		<div class="comment-tools">
			...
			?>
				<div class="comment-moderation">
					...
					<?php if('private' == $comment->comment_type ) { ?>
						<span class="private"><?php _e( 'Privater Kommentar', 'wmd_msreader' ); ?></a></span>
					<?php } elseif('0' == $comment->comment_approved ) { ?>
						<span class="approve"><a class="comment-approve" data-action="approve" href="#"><?php _e( 'Genehmigen', 'wmd_msreader' ); ?></a></span>
					<?php } else { ?>
						<span class="unapprove"><a class="comment-unapprove" data-action="unapprove" href="#"><?php _e( 'Nicht genehmigen', 'wmd_msreader' ); ?></a></span>
					<?php } ?>
					<?php if('private' != $comment->comment_type ) { ?>
					 | <span class="trash"><a class="comment-spam" href="#" data-action="spam"><?php _e( 'Spam', 'wmd_msreader' ); ?></a></span>
					 | <span class="trash"><a class="comment-trash" href="#" data-action="trash"><?php _e( 'Müll', 'wmd_msreader' ); ?></a>
					<?php } else { ?>
					 | <span class="delete"><a class="comment-trash" href="#" data-action="trash"><?php _e( 'Dauerhaft löschen', 'wmd_msreader' ); ?></a>
					<?php } ?>
				</div>
			<?php 
			}
			...
			<div class="reply"<?php echo ('0' == $comment->comment_approved) ? ' style="display:none"' : ''; ?>>
				<button class="comment-reply button button-secondary"><?php _e( 'Antworten', 'wmd_msreader' ); ?></button>
			</div>
			<?php 
			} 
			?>
...
<div id="msreader-no-comments">
	<?php _e( 'Noch keine Kommentare :(', 'wmd_msreader' ); ?>
</div>
<?php
}
?>